		...
		endgame(e) {
			...
			if (this.send == 0) {
				...
				if (this.$route.params.ids.length == 0) {
					const URL = import.meta.env.VITE_URL_BASE + "user/" + localStorage.getItem("pk") + "/endgame/?hostscore=" + this.player1Score + "&guestscore=" + this.player2Score + "&guest=" + "guest"
					axios.get(URL, {
						headers: {
					...
					const URL = import.meta.env.VITE_URL_BASE + "tournament/" + this.$route.params.ids[0] + "/match-result/?player1=" + this.$route.params.ids[1] + "&player1score=" + this.player1Score + "&player2=" + this.$route.params.ids[2] + "&player2score=" + this.player2Score
					axios.get(URL, {
						headers: {
							'Authorization': 'Bearer ' + localStorage.getItem("access")
						}
