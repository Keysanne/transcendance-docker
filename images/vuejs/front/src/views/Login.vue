<template>
    ...
    <div class="flex justify-content-center items-center">
        <div :class="signup_display ? 'hidden 2xl:flex' : 'flex'" class="flex-column align-items-center justify-content-center vh-100 w-[100vw] 2xl:w-[60vw]">
            ...
            <a :href="'https://api.intra.42.fr/oauth/authorize?client_id=u-s4t2ud-81d49bc94d932815eff49672933b8ab64b722a244e6fb161bdcc83d2a6807f0c&redirect_uri=https%3A%2F%2F' + ip + '%3A8080%2Flogin&response_type=code'" type="button" class="btn btn-dark w-[90%] max-w-xl mt-3 d-flex justify-content-center align-items-center h-10">
                {{ text.login_42[lang] }}
...
export default {
    data() {
        return {
            text: {
                ...
                signup: ["Sign up", "Créer mon compte", "サインアップ"],
                at_least: ["Username must be at least 5 characters", "Le nom d'utilisateur doit faire au moins 5 caractères", "ユーザー名は少なくとも 5 文字でなければなりません"],
                at_most: ["Username must be at most 14 characters", "Le nom d'utilisateur ne doit pas faire plus de 14 caractères", "ユーザー名は最大 14 文字でなければなりません"],
                alphanumeric: ["Username must be alphanumeric", "Le nom d'utilisateur doit être composé de lettres et/ou de chiffres", "ユーザー名は英数字でなければなりません"],
                ...
                already_used: ["Username already used", "Nom d'utilisateur déjà utilisé", "すでに使用されているユーザー名"],
                ...
                failed: ["Log in with 42 failed", "La connection avec 42 a échoué", "42 でログインに失敗しました"],
                no_account: ["Don't have an account?", "Vous n'avez pas encore de compte ?", "アカウントをお持ちではありませんか？"],
                already_account: ["Already have an account?", "Vous avez déjà un compte ?", "すでにアカウントをお持ちですか？"],
    ...
    methods: {
        ...
        login() {
	    	...
	    	const TOKEN_ENDPOINT = "api/token/"
            axios.post(import.meta.env.VITE_URL_BASE + CONNECT_ENDPOINT, {
                headers: {
                    'Content-Type': 'application/json'
                },
                params: {
    ...
    mounted() {
        ...
        if (window.location.search != '') {
            ...
            if (code != -1) {
                ...
                const URL = import.meta.env.VITE_URL_BASE + "user/remote-login/?code=" + code
                const TOKEN_ENDPOINT = "api/token/"
