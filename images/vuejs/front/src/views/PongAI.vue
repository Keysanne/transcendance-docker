...
export default {
	...
	methods: {
		...
		endgame(e) {
			...
			if (this.send == 0) {
				this.send = 1;
				const URL = import.meta.env.VITE_URL_BASE + "user/" + localStorage.getItem("pk") + "/endgame/?hostscore=" + this.player1Score + "&guestscore=" + this.player2Score + "&guest=" + "AI"
				axios.get(URL, {
					headers: {
		...
		movePlayer(e) {
			if (this.isstart == 0 && (e.code == "KeyW" || e.code == "KeyS")) {
				this.isstart = 1;
				this.gostart();
			}
		...
		detectCollision() {
			...
			if ((this.ball.y >= this.player1.y && this.ball.y <= this.player1.y + 80) && (this.ball.x <= this.player1.x + 10)) {
				...
				let x = this.ball.x;
				this.touchPoint = this.ball.y;
				while (x < 900){
					x = x + this.ball.velocityX;
					this.touchPoint = this.touchPoint + this.ball.velocityY;
				}
				if (this.touchPoint < 0 && this.touchPoint > -600)
					this.touchPoint = -this.touchPoint ;
				else if (this.touchPoint < -600 && this.touchPoint > -1200)
					this.touchPoint = 600 + (this.touchPoint + 600) ;
				else if (this.touchPoint > 600 && this.touchPoint < 1200)
					this.touchPoint = 600 - (this.touchPoint - 600) ;
				else if (this.touchPoint > 1200 && this.touchPoint < 1800)
					this.touchPoint = this.touchPoint - 1200 ;
		...
		AIMove() {
			if (this.player2.y + (this.playerHeight / 2) - this.ballHeight >= this.touchPoint) {
				if (this.player2.y - 10 + (this.playerHeight / 2) - this.ballHeight <= this.touchPoint || this.player2.y < 0) {
					return;
				}
				this.player2.y -=  10;
			}
			else if (this.player2.y + (this.playerHeight / 2) - this.ballHeight <= this.touchPoint) {
				if (this.player2.y + 10 + (this.playerHeight / 2) - this.ballHeight >= this.touchPoint || this.player2.y + this.playerHeight > 600) {
					return;
				}
				this.player2.y +=  10;
			}
